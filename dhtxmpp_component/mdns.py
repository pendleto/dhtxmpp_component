...
class mdns_service(object):
    ...
    def register_dht_with_mdns(self):
        ...
        port = 5678
        print("Registering dhtxmpp in mdns for ip %s with hostname %s" % (ip_addr, hostname))
    ...
    def listen_for_service(self):
        ...
        while(num_listen_tries<2):
            print("Listening for mdns dhtxmpp service...")
            num_listen_tries += 1          
            time.sleep(10)
            
            if self.service_address != None:
                print("Found mdns dhtxmpp service on %s" %(str(self.service_address)))
                break
